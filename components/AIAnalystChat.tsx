...
const AIAnalystChat: React.FC<AIAnalystChatProps> = ({ focusedHotspot }) => {
    ...
    const handleAgentMessage = (text: string, immediate: boolean = false) => {
        setIsTyping(true);
        setTimeout(() => {
            ...
            setIsTyping(false);
        }, immediate ? 500 : 1000 + Math.random() * 500); // Reduce delay if coming from async call
    };
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            // Generate Response
            const responseText = await generateAnalystResponse(input, focusedHotspot);

            // Add slight artificial delay if response was too fast, or just show it
            handleAgentMessage(responseText, true);
        } catch (error) {
            handleAgentMessage("I apologize, but I'm having trouble connecting to my analysis core. Please try again.", true);
        }
    };

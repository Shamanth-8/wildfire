...
const MapComponent: React.FC<MapComponentProps> = ({ allActiveFires, analyzedHotspots, focusedHotspot, customPrediction, onMapClick }) => {
  ...
  useEffect(() => {
    if (!mapRef.current) {
      mapRef.current = L.map('map', { zoomControl: false }).setView([20, 0], 2); // Global view
  ...
  useEffect(() => {
    ...
    const hotspotMarkers = analyzedHotspots.map(hotspot => {
        ...
        const popupContent = `
            <div class="p-1">
                <h3 class="font-bold text-md" style="color:${riskColor}">${hotspot.prediction?.riskLevel} Risk Zone</h3>
    ...
    if (customPrediction) {
        ...
        const popupContent = `
            <div class="p-1">
                <h3 class="font-bold text-md" style="color:${riskColor}">📍 Custom Analysis</h3>

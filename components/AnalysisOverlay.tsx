...
import { AnalyzedHotspot, RiskLevel, TimelineDataPoint } from '../types';
...
const AnalysisOverlay: React.FC<AnalysisOverlayProps> = ({ hotspot, onClose }) => {
    ...
    useEffect(() => {
        const loadTimeline = async () => {
            if (!hotspot.fireData.lat || !hotspot.fireData.lon) return;

            setIsLoadingTimeline(true);
            try {
                const data = await fetchTimelineData(hotspot.fireData.lat, hotspot.fireData.lon);
    ...
    return (
        <div className="absolute top-6 right-6 w-96 glass-panel rounded-xl shadow-2xl border border-white/10 z-50 animate-fade-in">
            ...
            <div className="p-5 space-y-6">
                ...
                <div className="pt-4 border-t border-white/5">
                    <TimelineChart data={timelineData} isLoading={isLoadingTimeline} />
                </div>
            </div>
        </div>
    );

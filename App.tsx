...
import GISRiskVisualization from './components/GISRiskVisualization';
import ValidationAccuracy from './components/ValidationAccuracy';
import LiveWildfireFeed from './components/LiveWildfireFeed';

const REFRESH_INTERVAL_MS = 5 * 60 * 1000; // 5 minutes
const MAX_HOTSPOTS_TO_ANALYZE = 10;

function App() {
  const [user, setUser] = useState<string | null>('DemoUser');
  ...
  const handleLogin = (username: string, lat: number, lon: number) => {
    setUser(username);
    // Immediately analyze the location provided from AuthPage
    analyzeCustomLocation(lat, lon, `${username}'s Location`);
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <aside className="w-full md:w-[450px] h-[40vh] md:h-full flex-shrink-0 glass-panel flex flex-col z-10 relative order-2 md:order-1 transition-all duration-300">
        <header className="p-6 border-b border-white/5 flex justify-between items-center relative overflow-hidden">
          {/* Subtle sheen animation */}
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent skew-x-12 translate-x-[-100%] animate-pulse-slow"></div>
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2.5 bg-gradient-to-br from-orange-500/20 to-red-600/10 rounded-xl border border-orange-500/30 shadow-[0_0_15px_rgba(249,115,22,0.2)]">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-orange-500 drop-shadow-[0_0_8px_rgba(249,115,22,0.8)]" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M12.395 2.553a1 1 0 00-1.45-.385c-.345.23-.614.558-.822.88-.214.33-.403.713-.57 1.116-.334.804-.614 1.768-.84 2.734a31.365 31.365 0 00-.613 3.58 2.64 2.64 0 01-.945-1.067c-.328-.68-.398-1.534-.398-2.654A1 1 0 005.05 6.05 6.981 6.981 0 003 11a7 7 0 1011.95-4.95c-.592-.591-.98-.985-1.348-1.467-.363-.476-.724-1.063-1.207-2.03zM12.12 15.12A3 3 0 017 13s.879.5 2.5.5c0-1 .5-4 1.25-4.5.5 1 .786 1.293 1.371 1.879A2.99 2.99 0 0113 13a2.99 2.99 0 01-.879 2.121z" clipRule="evenodd" />
                </svg>
              </div>
              <h1 className="text-2xl font-bold text-white tracking-tight font-sans">
                Wildfire<span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 glow-text-orange">Intel</span>
              </h1>
            </div>
          ...
          <div className="p-4 space-y-4">
            ...
            {customPrediction && (
              <div className="animate-fade-in">
                ...
                <GISRiskVisualization prediction={customPrediction} />
              </div>
            )}

            {/* Validation Accuracy */}
            <ValidationAccuracy metrics={accuracyMetrics} />

            {/* Visual Risk Intelligence (Image Upload) - Moved Here */}
            <div>
                 <ImageUploadAnalysis />
            </div>
            ...
            {/* Live Wildfire Feed */}
            <LiveWildfireFeed 
              hotspots={analyzedHotspots} 
              onSelect={setFocusedHotspot} 
            />
          </div>
        </div>
      </aside>
